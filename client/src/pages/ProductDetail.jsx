...
const ProductDetail = () => {
  ...
  const getAddressString = (addressObj) => {
    ...
    if (typeof addressObj === 'object') return addressObj.college || addressObj.city || 'India';
    try {
      const parsed = JSON.parse(addressObj);
      return parsed.college || parsed.city || 'India';
    } catch (e) {
      return addressObj;
    }
  };
  ...
  const getFullAddressString = (addressObj) => {
    if (!addressObj) return 'India';
    if (typeof addressObj === 'object') return [addressObj.college, addressObj.city, addressObj.state, addressObj.pincode].filter(Boolean).join(', ') || 'India';
    try {
      const parsed = JSON.parse(addressObj);
      return [parsed.college, parsed.city, parsed.state, parsed.pincode].filter(Boolean).join(', ') || 'India';
    } catch (e) {
      return addressObj;
    }
  };
  ...
  return (
    <div className={`pm-wrapper deccan-theme ${theme}-mode`}>
      ...
      <main className="product-detail-main deccan-main-layout">
        {loading ? (
          <div className="loading-state">
            <div className="deccan-spinner"></div>
            <p style={{ margin: '16px 0 0 0', fontWeight: '600', color: 'var(--deccan-text-secondary)', fontSize: '15px' }}>
              Retrieving secure verified listings...
            </p>
          </div>
        ) : !product ? (
          <div className="error-state">
            <h2>Product listing node offline</h2>
            <p style={{ color: 'var(--deccan-text-secondary)', marginBottom: '24px' }}>
              The signature or address details for this listing could not be resolved.
            </p>
            <button onClick={() => navigate('/')}>Return to Marketplace</button>
          </div>
        ) : (
          <div className="product-container deccan-layout-grid">
            
            {/* Left Column: Image box and Description details */}
            <div className="product-left-column deccan-left-col">
              <div className="product-image-container deccan-image-box">
                <img 
              ...
              <div className="product-description-section deccan-desc-card">
                <h3>Product Specifications</h3>
                
                <div style={{ marginBottom: '24px' }}>
                  <span className="detail-category">
                    Category: {product.category || 'Classified Ad'}
                  </span>
                </div>

                <div className="deccan-specs-dashboard">
                  <div className="deccan-spec-node">
                    <span className="label">Verification State</span>
                    <span className="val glow-green">✓ Deccan Verified</span>
                  </div>
                  <div className="deccan-spec-node">
                    <span className="label">Listing ID</span>
                    <span className="val">#PM-{String(product._id || product.id || '').slice(-6).toUpperCase() || 'N0DE'}</span>
                  </div>
                  <div className="deccan-spec-node">
                    <span className="label">Listing Tier</span>
                    <span className="val">Deccan AI Elite Partner</span>
                  </div>
                  <div className="deccan-spec-node">
                    <span className="label">Clarity Index</span>
                    <span className="val glow-green">99.9% High Fidelity</span>
                  </div>
                </div>

                <div style={{ color: 'var(--deccan-text-muted)', fontSize: '11px', fontWeight: '800', textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: '10px' }}>
                  Product Statement & Details
                </div>
            ...
            <div className="product-info-sidebar deccan-sidebar">
              
              {/* Product title & price card */}
              <div className="deccan-price-box">
                <div className="detail-price-row" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <h1 className="detail-price">
                    {String(product.price || '0').startsWith('₹') ? product.price : `₹${product.price || '0'}`}
                  </h1>
              ...
              {/* Seller details card */}
              <div className="deccan-seller-box">
                <h3>Certified Partner Profile</h3>
                <div className="seller-profile">
                  ...
                  <div className="seller-profile-info">
                    ...
                    <p className="seller-member-since">Verified Member since Jan 2024</p>
                  </div>
                </div>
                
                {product.show_mobile && product.mobile_no && (
                  <div className="seller-mobile-block" style={{ marginBottom: '20px' }}>
              ...
              <div className="deccan-location-box">
                <h3>Geographical Target Locked</h3>
                <p>
                  📍 Coordinates: {getFullAddressString(product.user_address)}
                </p>
      ...
      {/* Official Deccan Experts Styled 4-Column Footer */}
      <footer className="pm-footer deccan-footer">
        <div className="deccan-footer-peaks-bg"></div>
        <div className="pm-footer-top-columns" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
          <div className="pm-footer-column">
            <h4>About Us</h4>
            <ul className="pm-footer-links-list" style={{ listStyle: 'none', padding: 0 }}>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>About PuranaMall Group</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Careers</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Contact Us</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>PuranaMall People</a></li>
            </ul>
          </div>
          <div className="pm-footer-column">
            <h4>PuranaMall</h4>
            <ul className="pm-footer-links-list" style={{ listStyle: 'none', padding: 0 }}>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Help Center</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Sitemap</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Legal & Privacy Information</a></li>
              <li><a href="#" onClick={(e) => { e.preventDefault(); navigate('/'); }}>Vulnerability Disclosure Program</a></li>
            </ul>
          </div>
          <div className="pm-footer-column">
            <h4>Follow Us</h4>
            <div className="pm-footer-social-wrap">
              <a href="#" className="pm-footer-social-icon facebook" title="Facebook" onClick={(e) => e.preventDefault()}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
                  <path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.84 3.44 8.87 8 9.8V15H8v-3h2V9.5C10 7.57 11.57 6 13.5 6H16v3h-2c-.55 0-1 .45-1 1v2h3v3h-3v6.95c4.56-.93 8-4.96 8-9.75z"/>
                </svg>
              </a>
              <a href="#" className="pm-footer-social-icon twitter" title="Twitter / X" onClick={(e) => e.preventDefault()}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
                </svg>
              </a>
              <a href="#" className="pm-footer-social-icon instagram" title="Instagram" onClick={(e) => e.preventDefault()}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
              ...
              <a href="#" className="pm-footer-social-icon youtube" title="YouTube" onClick={(e) => e.preventDefault()}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
              ...
              <a href="#" className="pm-footer-social-icon linkedin" title="LinkedIn" onClick={(e) => e.preventDefault()}>
                <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
        ...
        <div className="pm-footer-bottom-strip">
          <div className="pm-footer-bottom-container" style={{ display: 'flex', justifyContent: 'center' }}>
